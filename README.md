...
购房/租房决策辅助工具 -- 一张可编辑的房源对比大表格，替代你看房时的 Excel 和备忘录。
...
- 贝壳/链家/自如的收藏只是浏览历史，无法编辑数据、添加自己的备注
- "主卧西晒"、"房东急售可砍价"这些关键信息没地方结构化沉淀
- 买房和租房的候选房源分散管理，没有统一的对比工具

## 核心功能

| 功能 | 说明 |
| --- | --- |
| **买房/租房切换** | 顶部一键切换买房/租房模式，数据独立管理，价格单位自动适配（万元/元月），AI 识别 prompt 自动切换，模式偏好持久化到用户资料 |
...
| **截图识别** | 支持多张截图批量识别，AI 并行提取后由用户确认再入库，后台执行不阻塞操作，失败可重试；接口要求登录、限制频率并校验输入输出 |
| **免登录示例** | 落地页提供可交互的真实房源对比示例，访客无需注册即可理解产品价值 |
| **增长埋点** | 记录落地页体验、注册、添加房源、达到 1/3 套、开始对比和截图确认等核心漏斗事件 |
...
| AI 多模态模型 | 截图识别提取房源数据（兼容 OpenAI 格式接口，可切换供应商，支持买房/租房截图） |
...
新用户注册后进入空房源状态，可通过手动添加或截图识别开始录入；登录前可在落地页操作示例对比表。
...
- [x] 截图识别提取房源，用户确认后入库（AI 多模态模型）
- [x] 核心增长漏斗事件埋点
- [x] 图片上传（Supabase Storage）
- [x] 移动端适配（手机竖屏全屏详情/对比弹窗、响应式表头）
- [x] 买房/租房模式切换（价格单位适配、AI识别适配、模式持久化）
